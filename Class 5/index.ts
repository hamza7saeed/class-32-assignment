...
let english = 95;
let science = 79;
let per = (math + english + science)/3;
...
*Objective:* Develop a program that utilizes compound expressions to make 
decisions based on multiple conditions.

*Task:*
Write a JavaScript program that takes hardcoded values for a 
student's marks in two subjects (Math and English) and determines eligibility for an
...
This assignment focuses on the use of compound expressions to make decisions based on multiple
 conditions, reinforcing the understanding of logical operators and their combination 
 in programming.*/

 let Mmath = 80;
 let English = 80;

 if (Mmath >= 80 && English >= 75){
    console.log("Student is eligible for the advanced placement course");
    
 } else {
    console.log("Student is not eligible for the advanced placement course");
...
This assignment emphasizes the use of nested if 
statements to handle multiple conditions and decisions in 
a structured manner. 
It also introduces the concept of eligibility for voting based on age and nationality.*/
...
console.log("Your are eligible for vote");
    ...
    console.log("You are international immigrant");
